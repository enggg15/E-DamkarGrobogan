...
class AdminController extends Controller
{
    ...
    {
        $title = 'Kelola Admin | E-Damkar Grobogan';
    ...
    public function store(Request $request)
    {
        ...
        $admin->kedudukans_id = $request->kedudukans_id;
        $admin->save();

        $input = user_listData::create([
            'username' => $request->email,
            'password' => Hash::make($request->input('password')),
            'namaLengkap' => $request->nama_admin,
            'noHp' => $request->noHp,
            'kodeOtp' => rand(0, 99999),
            'status' => 'Verified',
            'foto_user' => null,
        ]);
